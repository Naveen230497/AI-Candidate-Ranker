...
def run_pipeline(candidates_path: str, output_path: str, is_sample: bool = False):
    ...
    for candidate in tqdm(filtered_candidates, desc="  -> Scoring", unit=" candidates"):
        ...
        if len(top_k_heap) < RE_RANK_LIMIT:
            heapq.heappush(top_k_heap, (stage1_score, candidate['candidate_id'], entry))
        else:
            heapq.heappushpop(top_k_heap, (stage1_score, candidate['candidate_id'], entry))
    
    print(f"\n[4/7] Filtering top {RE_RANK_LIMIT:,} candidates for AI Re-ranking...")
    
    # Extract from heap and sort descending
    top_k_candidates = [item[2] for item in sorted(top_k_heap, key=lambda x: x[0], reverse=True)]
